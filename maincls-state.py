...
if __name__ == '__main__':
    ...
    def train_loop(model, data, optimizer, num_epochs, model_name='', city_name='',save_path=None):
        ...
        all_predict_labels= []
        print(f'now training {model_name}  for {city_name}\n')
        print('*'*20)
...
#                 plt.show()
    ...
    # %%time
    for e in [ 'California','Oregon','Utah','Maryland','Minnesota','Connecticut']:

    
        state_abbrev = us_state_to_abbrev[e]
        city_format = e
        city_name = e
        if os.path.exists(file_path+state_abbrev+'/processed'):
            shutil.rmtree(file_path+state_abbrev+'/processed')
        # dataset = TRAVELDataset(file_path, city_name+'_'+state_abbrev)
        dataset = TRAVELDataset(file_path, state_abbrev)
        data = dataset[0]
